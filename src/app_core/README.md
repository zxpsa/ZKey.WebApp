...
| 日期        | 作者   | 内容          |
| --------- | ---- | ----------- |
| 2017-2-28 | 裴胜   | 新增基础服务相关接口  |
| 2017-1-7  | 裴胜   | 优化接口        |
| 2016-9-21 | 裴胜   | 创建前端支持文档1.0 |

## 说明

- 手机端与服务端采用Http/Https通信

- 采用cookie/sessionStorage保存token值，通过heard中添加token进行传输

- 请求与返回数据格式均为JSON字符串

- Api接口相对路径：/api/{0}/{1}，其中0和1为不同接口动态变化的值

- 开发服host: http://10.10.7.104
...
- 禁止modules下第一层模块间的(独立功能模块)相互依赖(避免单次过多的加载代码) 每一层模块公共代码请放置于common中
...
- 大量使用了开源的第三方控件进行二次封装,故对非本人的代码,所有权归原作者所有。
