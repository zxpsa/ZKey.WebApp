...
<script lang="js">
	...
	var h5DirectApp = require('app_core/services/pay_service/h5_direct_app_services');
	var initTime = 60;
	module.exports = {
		//数据模型
		data: function() {
...
//				是否支持荣宝
	 ...
	 */
	function show(selectBankCallback, smsSuer, reSendSMS, bankType, canUseBankType,cannel) {
		var vm = this;
		vm.showing = true;
	...
	function toAddBankPage() {
		var onZKBack = window.onZKBack;
		//返回时加载刷新方法
		window.onZKBack = function() {
			//返回刷新页面
			vm.refresh(vm);
			window.onZKBack = onZKBack;
		}
		var vm = this;
		//荣宝银行卡
		if(vm.canUseBankType == 1) {
